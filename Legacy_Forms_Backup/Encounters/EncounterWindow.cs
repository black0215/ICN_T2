...
namespace Albatross.Forms.Encounters
{
    public partial class EncounterWindow : Form
    {
        ...
        private (int level, int maxLevel, int weight, int unk1, int unk2, int unk4, float unk5, int unk7) ExtractAllVariables(IEncountChara chara)
        {
            ...
            {
                return (spot.MaxLevel, spot.Level, spot.Weight, spot.Unk1, spot.Unk2, spot.Unk4, spot.Unk5, spot.Unk7);
            }
            ...
            if (chara is YKW2.EncountChara enc)
            {
                ...
                return (
                    ...
                    enc.Unk1,
                    enc.Unk2,
                    type.GetProperty("Unk3")?.GetValue(enc) as int? ?? 0,
                    type.GetProperty("Unk4")?.GetValue(enc) as float? ?? 0f,
                    type.GetProperty("Unk5")?.GetValue(enc) as int? ?? 0
                );
            }
        ...
        private Bitmap GetYokaiImage(ICharabase charabase)
        {
            ...
            try
            {
                ...
                if (GameOpened.Files["face_icon"].File.Directory.FileExists(fullPath))
                {
        ...
        private void UpdateCharaVariable(IEncountChara chara, int columnIndex, string valueStr)
        {
            ...
            if (chara is YKW2.YokaiSpotChara spot)
            {
                switch (columnIndex)
                {
                    case 2: if (int.TryParse(valueStr, out int v2)) spot.MaxLevel = v2; break;
                    case 3: if (int.TryParse(valueStr, out int v3)) spot.Level = v3; break;
                    case 4: if (int.TryParse(valueStr, out int v4)) spot.Weight = v4; break;
            ...
            if (chara is YKW2.EncountChara enc)
            {
                ...
                switch (columnIndex)
                {
                    case 2: if (int.TryParse(valueStr, out int v2)) enc.MaxLevel = v2; break;
                    case 3: if (int.TryParse(valueStr, out int v3)) enc.Level = v3; break;
                    case 4: if (int.TryParse(valueStr, out int v4)) enc.Weight = v4; break;
                    ...
                    case 8:
                        ...
                        if (prop8 != null && float.TryParse(valueStr, out float v8)) prop8.SetValue(enc, v8);
                        break;
                    case 9:
                        var prop9 = type.GetProperty("Unk5");
                        if (prop9 != null && int.TryParse(valueStr, out int v9)) prop9.SetValue(enc, v9);
                        break;
                }
            }
        }
        ...
        private void EncounterDataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (EncounterDataGridEditInProgress && !IsProcessingCellValueChange)
            {
                ...
                if (SelectedEncountTable.EncountOffsets[e.RowIndex] != 0 && SelectedEncountTable.EncountOffsets[e.RowIndex] != -1)
                {
                    // Update Existing
                    ...
                    else if (e.ColumnIndex == 2) currentChara.MaxLevel = level; // Note: This logic seems to couple with Col 3 update in original code
