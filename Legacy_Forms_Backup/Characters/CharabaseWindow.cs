...
namespace Albatross.Forms.Characters
{
    public partial class CharabaseWindow : Form
    {
        ...
        // [FIX] Add parameterless constructor for Visual Studio Designer
        public CharabaseWindow()
        {
            InitializeComponent();
        }
        ...
        public CharabaseWindow(IGame game)
        {
            ...
            // [FIX] Ensure GameOpened is valid before loading
            if (GameOpened != null)
            {
                LoadCharabase();
            }
        ...
        private string[] GetNames(ICharabase[] charabases)
        {
            return charabases
                ...
                {
                    if (Charanames != null && Charanames.Nouns.TryGetValue(charabase.NameHash, out var noun) && noun.Strings.Count > 0)
        ...
        {
            if (FaceIcon == null) return new Bitmap(size, size);
        ...
        private void SetMedal(bool draw)
        {
            if (draw)
            {
                ...
                // Set medal
                int size = 44;
                if (GameOpened.Name == "Yo-Kai Watch 3") size = 32;

                if (medalPictureBox != null)
                {
                    medalPictureBox.Image = CropMedal(x, y, size);
                }
            }
            else
            {
        ...
        private void LoadCharabase()
        {
            ...
            // [FIX] Safety check for face_icon
            if (GameOpened.Files.ContainsKey("face_icon"))
            {
                try
                {
                    var file = GameOpened.Files["face_icon"];
                    byte[] faceIconData = file.File.Directory.GetFileFromFullPath(file.Path + "/face_icon.xi");
                    FaceIcon = IMGC.ToBitmap(faceIconData);
                }
                catch
                {
                    FaceIcon = new Bitmap(1, 1);
                }
            }
            else
            {
            ...
            // [FIX] Safety check for chara_text
            if (GameOpened.Files.ContainsKey("chara_text"))
            {
                try
                {
                    var file = GameOpened.Files["chara_text"];
                    Charanames = new T2bþ(file.File.Directory.GetFileFromFullPath(file.Path));
                }
                catch
                {
                    Charanames = new T2bþ(); // Empty fallback
                }
            }
            else
            {
            ...
            // [FIX] Safety check for model folder
            if (GameOpened.Files.ContainsKey("model"))
            {
                try
                {
                    var modelFile = GameOpened.Files["model"];
                    var modelFolder = modelFile.File.Directory.GetFolderFromFullPath(modelFile.Path);
                    if (modelFolder != null && modelFolder.Folders != null)
                    {
                        modelFlatComboBox.Items.AddRange(modelFolder.Folders.Select(x => x.Name).ToArray());
                    }
                }
                catch { }
            }
            ...
            else if (GameOpened.Name == "Yo-Kai Watch 3")
            {
                ...
                if (GameOpened.Files.ContainsKey("chara_desc_text"))
                {
                    try
                    {
                        var file = GameOpened.Files["chara_desc_text"];
                        Charadesc = new T2bþ(file.File.Directory.GetFileFromFullPath(file.Path));
                    }
                    catch
                    {
                        Charadesc = new T2bþ();
        ...
        private void CharabaseWindow_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            if (Charanames != null && GameOpened.Files.ContainsKey("chara_text"))
        ...
        private void InsertToolStripMenuItem_Click(object sender, EventArgs e)
        {
            using (NewCharabaseWindow newCharabaseWindow = new NewCharabaseWindow(GameOpened, Charanames, modelFlatComboBox.Items.OfType<string>().ToArray()))
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    if (lastIndex < 0) lastIndex = Charabases.Count;
                    else if (lastIndex > Charabases.Count) lastIndex = Charabases.Count;
                    ...
                    if (lastIndex < characterListBox.Items.Count)
        ...
        private void CharacterListBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (CharabasesFiltred != null && CharabasesFiltred.Count > 0)
            {
                if (characterListBox.SelectedIndex < CharabasesFiltred.Count)
                    SelectedCharabase = CharabasesFiltred[characterListBox.SelectedIndex];
            }
            else
            {
                if (characterListBox.SelectedIndex < Charabases.Count)
                    SelectedCharabase = Charabases[characterListBox.SelectedIndex];
            }

            if (SelectedCharabase == null) return;

            // hashTextBox (User code had this, assuming it exists in Designer)
            // If compile error, I will fix.
            if (Controls.Find("hashTextBox", true).Length > 0)
                ((TextBox)Controls.Find("hashTextBox", true)[0]).Text = SelectedCharabase.BaseHash.ToString("X8");

            if (Charanames != null && Charanames.Nouns != null && Charanames.Nouns.ContainsKey(SelectedCharabase.NameHash))
            {
            ...
            if (GameOpened.Name == "Yo-Kai Watch 3" && Charadesc != null && Charadesc.Texts.ContainsKey(SelectedCharabase.DescriptionHash))
            {
            ...
            else if (Charanames != null && Charanames.Texts.ContainsKey(SelectedCharabase.DescriptionHash))
            {
        ...
        {
            // 플레이스홀더 텍스트인 경우 무시
            if (searchTextBox.Text == "검색...") return;

            // "검색..." 텍스트가 섞여 들어간 경우 제거 (지바냥검색... 문제 해결)
            if (searchTextBox.Text.Contains("검색..."))
            {
                searchTextBox.Text = searchTextBox.Text.Replace("검색...", "");
                searchTextBox.ForeColor = Color.White;
                searchTextBox.SelectionStart = searchTextBox.Text.Length; // 커서 끝으로 이동
            }

            // 텍스트 색상이 회색(플레이스홀더 색)이라면 흰색으로 변경 (입력 시작됨)
            if (searchTextBox.ForeColor == Color.Gray && !string.IsNullOrEmpty(searchTextBox.Text))
            {
                searchTextBox.ForeColor = Color.White;
            }
            ...
            else
            {
                CharabasesFiltred = Charabases
                    .Where(charabase =>
                    {
                        ...
                        bool searchByYokaiName = Charanames != null && Charanames.Nouns.ContainsKey(charabase.NameHash) &&
        ...
        {
            if (!GameOpened.Files.ContainsKey("chara_text")) return;
        ...
        private void DescriptionTextBox_Click(object sender, EventArgs e)
        {
            ...
            if (!GameOpened.Files.ContainsKey(fileName)) return;
        ...
        private void characterGroupBox_Enter(object sender, EventArgs e)
        {
