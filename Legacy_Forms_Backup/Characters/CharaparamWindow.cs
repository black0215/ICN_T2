...
namespace Albatross.Forms.Characters
{
    public partial class CharaparamWindow : Form
    {
        ...
        public CharaparamWindow(IGame game)
        {
            ...
            // Search filtering setup
            searchTextBox.Enter += SearchTextBox_Enter;
            searchTextBox.Leave += SearchTextBox_Leave;

            LoadCharaparam();
        }
        private void LogComboBoxChange(string comboBoxName, string dictionaryType, object selectedItem, int oldHash, int newHash)
        {
            Console.WriteLine("\n========================================");
            Console.WriteLine($"[ComboBox] 변경 감지: {DateTime.Now:HH:mm:ss.fff}");
            Console.WriteLine("========================================");
            Console.WriteLine($"[ComboBox] 🎯 컨트롤: {comboBoxName}");
            Console.WriteLine($"[ComboBox] 📚 연결된 딕셔너리: {dictionaryType}");
            Console.WriteLine($"[ComboBox] 📝 선택된 아이템: {selectedItem?.ToString() ?? "(null)"}");
            Console.WriteLine($"[ComboBox] 🔄 해시 변경:");
            Console.WriteLine($"   이전: 0x{oldHash:X8} ({oldHash})");
            Console.WriteLine($"   이후: 0x{newHash:X8} ({newHash})");
            Console.WriteLine("========================================\n");
        }

        private void LogTextFileLoad(string fileName, T2bþ textObject, string source)
        {
            Console.WriteLine("\n========================================");
            Console.WriteLine($"[LoadText] 파일 로드: {DateTime.Now:HH:mm:ss.fff}");
            Console.WriteLine("========================================");
            Console.WriteLine($"[LoadText] 📄 파일명: {fileName}");
            Console.WriteLine($"[LoadText] 🔗 로드 방식: {source}");
            Console.WriteLine($"[LoadText] 🔤 인코딩: {textObject.Encoding.EncodingName} ({textObject.Encoding.CodePage})");
            Console.WriteLine($"[LoadText] 📊 데이터:");
            Console.WriteLine($"   Texts: {textObject.Texts.Count}개");
            Console.WriteLine($"   Nouns: {textObject.Nouns.Count}개");

            // 샘플 데이터 출력
            if (textObject.Texts.Count > 0)
            {
                var samples = textObject.Texts.Take(3);
                Console.WriteLine($"[LoadText] 📝 Texts 샘플 (처음 3개):");
                foreach (var sample in samples)
                {
                    var text = sample.Value.Strings.FirstOrDefault()?.Text;
                    var preview = text?.Length > 30 ? text.Substring(0, 30) + "..." : text;
                    Console.WriteLine($"   0x{sample.Key:X8} = \"{preview}\"");
                }
            }

            if (textObject.Nouns.Count > 0)
            {
                var samples = textObject.Nouns.Take(3);
                Console.WriteLine($"[LoadText] 📝 Nouns 샘플 (처음 3개):");
                foreach (var sample in samples)
                {
                    var text = sample.Value.Strings.FirstOrDefault()?.Text;
                    var preview = text?.Length > 30 ? text.Substring(0, 30) + "..." : text;
        ...
        private void LoadCharaparam()
        {
            ...
            if (!GameOpened.Files.ContainsKey("item_text"))
            {
                MessageBox.Show(
                    "item_text 파일을 찾을 수 없습니다.\nLanguage 파일이 손상되었거나 저장 과정에서 누락되었습니다.",
                    "필수 파일 누락",
            ...
            // Get yokais and evolutions
            // [FIX] Load Bosses (stored as NPCs) by including matching NPC bases
            Charabases.AddRange(GameOpened.GetCharacterbase(true));

            // Load params first to filter NPCs
            var loadedParams = GameOpened.GetCharaparam();
            Charaparams.AddRange(loadedParams);

            // Add NPCs only if they have a matching parameter (e.g. Bosses)
            var npcBases = GameOpened.GetCharacterbase(false);
            // [OPTIMIZATION] Use HashSet for O(1) lookup
            var loadedParamHashes = new HashSet<int>(loadedParams.Select(p => p.BaseHash));

            // Add NPCs only if they have a matching parameter (e.g. Bosses)
            // Filtering is now O(N) instead of O(N*M)
            var bossBasesFiltered = npcBases.Where(npc => loadedParamHashes.Contains(npc.BaseHash));

            // Use another HashSet to track existing bases in Charabases for fast duplication check
            var existingBaseHashes = new HashSet<int>(Charabases.Select(c => c.BaseHash));

            foreach (var boss in bossBasesFiltered)
            {
                if (existingBaseHashes.Add(boss.BaseHash)) // Returns true if added (unique)
                {
                    Charabases.Add(boss);
                }
            }
        ...
        private void CharaparamWindow_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            using (WaitForm wait = new WaitForm("저장 중입니다...\n(데이터가 많아 시간이 걸릴 수 있습니다)"))
            {
                wait.Show(); // 모달이 아닌 Show()로 실행하여 메인 스레드 블로킹 최소화 (단, 여기선 어차피 동기 작업)
        ...
        {
            if (!searchTextBox.Focused || searchTextBox.Text == "검색...") return;

            // "검색..." 텍스트가 섞여 들어간 경우 제거
            if (searchTextBox.Text.Contains("검색..."))
            {
                searchTextBox.Text = searchTextBox.Text.Replace("검색...", "");
                searchTextBox.ForeColor = Color.White;
        ...
        private void AttackFlatComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            int oldHash = SelectedCharaparam.AttackHash;
            int newHash = 0;
            ...
            else
            {
                if (GameOpened.Name == "Yo-Kai Watch 3")
                {
                    newHash = Skills[attackFlatComboBox.SelectedIndex].SkillHash;
                    dictionaryType = "Skills";
                }
                else
                {
                    newHash = BattleCommands[attackFlatComboBox.SelectedIndex].BattleCommandHash;
                    dictionaryType = "BattleCommands";
                }
            }

            LogComboBoxChange("attackFlatComboBox", dictionaryType, attackFlatComboBox.SelectedItem, oldHash, newHash);
            SelectedCharaparam.AttackHash = newHash;
        }
        ...
        private void TechniqueFlatComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            int oldHash = SelectedCharaparam.TechniqueHash;
            int newHash = 0;
            string dictionaryType = "";
            ...
            else
            {
                if (GameOpened.Name == "Yo-Kai Watch 3")
                {
                    newHash = Skills[techniqueFlatComboBox.SelectedIndex].SkillHash;
                    dictionaryType = "Skills";
                }
                else
                {
                    newHash = BattleCommands[techniqueFlatComboBox.SelectedIndex].BattleCommandHash;
                    dictionaryType = "BattleCommands";
                }
            }

            LogComboBoxChange("techniqueFlatComboBox", dictionaryType, techniqueFlatComboBox.SelectedItem, oldHash, newHash);
            SelectedCharaparam.TechniqueHash = newHash;
        }
        ...
        private void InspiritFlatComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            int oldHash = SelectedCharaparam.InspiritHash;
            int newHash = 0;
            string dictionaryType = "";
            ...
            else
            {
                if (GameOpened.Name == "Yo-Kai Watch 3")
                {
                    newHash = Skills[inspiritFlatComboBox.SelectedIndex].SkillHash;
                    dictionaryType = "Skills";
                }
                else
                {
                    newHash = BattleCommands[inspiritFlatComboBox.SelectedIndex].BattleCommandHash;
                    dictionaryType = "BattleCommands";
                }
            }

            LogComboBoxChange("inspiritFlatComboBox", dictionaryType, inspiritFlatComboBox.SelectedItem, oldHash, newHash);
            SelectedCharaparam.InspiritHash = newHash;
        }
        ...
        private void GuardFlatComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            int oldHash = SelectedCharaparam.GuardHash;
            int newHash = 0;
            string dictionaryType = "";
            ...
            else
            {
                if (GameOpened.Name == "Yo-Kai Watch 3")
                {
                    newHash = Skills[guardFlatComboBox.SelectedIndex].SkillHash;
                    dictionaryType = "Skills";
                }
                else
                {
                    newHash = BattleCommands[guardFlatComboBox.SelectedIndex].BattleCommandHash;
                    dictionaryType = "BattleCommands";
                }
            }

            LogComboBoxChange("guardFlatComboBox", dictionaryType, guardFlatComboBox.SelectedItem, oldHash, newHash);
            SelectedCharaparam.GuardHash = newHash;
        }
        ...
        private void SoultimateFlatComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            int oldHash = SelectedCharaparam.SoultimateHash;
            int newHash = 0;
            string dictionaryType = "";
            ...
            else
            {
                if (GameOpened.Name == "Yo-Kai Watch 3")
                {
                    newHash = Skills[soultimateFlatComboBox.SelectedIndex].SkillHash;
                    dictionaryType = "Skills";
                }
                else
                {
                    newHash = BattleCommands[soultimateFlatComboBox.SelectedIndex].BattleCommandHash;
                    dictionaryType = "BattleCommands";
                }
            }

            LogComboBoxChange("soultimateFlatComboBox", dictionaryType, soultimateFlatComboBox.SelectedItem, oldHash, newHash);
            SelectedCharaparam.SoultimateHash = newHash;
        }
        ...
        private void AbilityFlatComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            int oldHash = SelectedCharaparam.AbilityHash;
            int newHash = 0;
            ...
            {
                newHash = Abilities[abilityFlatComboBox.SelectedIndex].CharaabilityConfigHash;
            }

            LogComboBoxChange("abilityFlatComboBox", "Abilities", abilityFlatComboBox.SelectedItem, oldHash, newHash);
            SelectedCharaparam.AbilityHash = newHash;
        }
