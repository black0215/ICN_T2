...
namespace Albatross
{
    public class WaitForm : Form
    {
        ...
        public WaitForm(string message = "저장 중입니다...")
        {
            ...
            // === 컨트롤 추가 ===
            // [FIX] Add Bottom panel FIRST so it reserves space, then Fill label
        ...
        public void SetProgress(int current, int total, string currentFileName)
        {
            ...
            try
            {
                ...
                ProgressLabel.Text = $"진행 상황 ({current}%)\n{currentFileName}";

                // [FIX] Force UI Update immediately
                ProgressLabel.Update();
                ProgressBar.Update();
                Application.DoEvents(); // Process pending messages to ensure redraw
            }
            catch { }
        }
    }
