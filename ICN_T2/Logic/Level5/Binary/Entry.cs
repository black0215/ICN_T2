...
using ICN_T2.Tools;       // Was Albatross.Tools
using ICN_T2.Tools.IO;    // BinaryDataWriter
using ICN_T2.Logic.Level5.Binary; // For Variable, EntryType
...
namespace ICN_T2.Logic.Level5.Binary
{
    public class Entry
    {
        ...
        public Dictionary<int, string> GetStrings()
        {
            ...
            {
                if (variable.Type == EntryType.String) // Logic.Type.String -> EntryType.String
                {
                    OffsetTextPair offsetTextPair = variable.Value as OffsetTextPair;

                    // [FIX] potential null check
                    if (offsetTextPair != null)
                    {
        ...
        // Clone 메소드 내 생성자 호출 부분도 UTF-8 유지 확인
        public Entry Clone()
        {
            // [FIX] Variable copy constructor might be needed, or create new Variable manually
            List<Variable> clonedVariables = Variables.Select(v => new Variable(v.Type, v.Value)).ToList();
            List<Entry> clonedChildren = Children.Select(child => child.Clone()).ToList();

            return new Entry(Name, clonedVariables, Encoding.UTF8)
            {
                EndTerminator = EndTerminator,
        ...
        private byte[] EncodeTypes(List<EntryType> types) // List<Type> -> List<EntryType>
        {
        ...
        public byte[] EncodeEntry()
        {
            using (MemoryStream memoryStream = new MemoryStream())
            {
                using (BinaryDataWriter writer = new BinaryDataWriter(memoryStream))
                {
                    ...
                    List<EntryType> types; // List<Type> -> List<EntryType>
        ...
        // GetEncoding 메소드: 무조건 UTF-8 반환
        public byte GetEncoding() => 1;
        ...
        private void UpdateOffsetsRecursive(Dictionary<int, int> newOffsets)
        {
            foreach (Variable variable in Variables)
            {
                if (variable.Type == EntryType.String)
                {
                    OffsetTextPair offsetTextPair = variable.Value as OffsetTextPair;
        ...
        public void SetVariablesFromClass<T>(T structure) where T : class
        {
            ...
            foreach (var property in properties)
            {
                ...
                EntryType variableType; // Type -> EntryType
                ...
                if (propertyValue != null)
                {
                    if (propertyType.IsArray)
                    {
                        ...
                        {
                            EntryType variableTypeElementFromArray;

                            switch (System.Type.GetTypeCode(arrayElements[i].GetType()))
                            {
                                case TypeCode.String:
                                    variableTypeElementFromArray = EntryType.String;
                                    break;
                                case TypeCode.Int32:
                                    variableTypeElementFromArray = EntryType.Int;
                                    break;
                                case TypeCode.Single:
                                    variableTypeElementFromArray = EntryType.Float;
                                    break;
                    ...
                    else
                    {
                        ...
                        {
                            case TypeCode.String:
                                variableType = EntryType.String;
                                break;
                            case TypeCode.Int32:
                                variableType = EntryType.Int;
                                break;
                            case TypeCode.Single:
                                variableType = EntryType.Float;
                                break;
                            case TypeCode.Boolean:
                                variableType = EntryType.Int;
                                break;
